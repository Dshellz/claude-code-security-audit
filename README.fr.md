...
ajoute une commande **`/audit`** qui **auto-détecte ta stack**, applique la checklist de
...
```
/audit                          # audite les changements en cours (git diff), auto-détection, rapport FR
/audit en                       # idem, rapport en anglais
/audit fr src/Controller/Foo.php   # audite un fichier précis, rapport FR
```
...
`/audit` repère les fichiers marqueurs à la racine du projet : `composer.json` → PHP,
...
└── security-audit/               # le plugin
    ...
    ├── commands/audit.md         # /audit (auto-détection)
    └── rules/
